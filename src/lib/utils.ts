 ...
 * set. Returns the variant id or `undefined` when no match.
 *
 * Matches regardless of option order, case-insensitive on values.
 */
export function getVariantId(
  product: Pick<Product, "variants">,
  selectedOptions: Array<{ name: string; value: string }> | Record<string, string>,
): string | undefined {
...
export function isVariantAvailable(
  variant: Pick<Variant, "availableForSale" | "quantityAvailable"> | null | undefined,
): boolean {
