...
export function Footer({ className }: FooterProps) {
  return (
    <footer
      className={cn("mt-0 bg-brand-950/95 border-t border-brand-200/10 text-slate-300", className)}
    >
      <div className="container-shop py-14">
        <div className="grid gap-10 md:grid-cols-[1.2fr_2fr_1.3fr]">
          ...
          <div>
            ...
            <p
              className="mt-5 text-sm max-w-xs leading-relaxed text-slate-400"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-8">
            {SECTIONS.map((section) => (
              <div key={section.heading}>
                <h3
                  className="text-xs font-semibold uppercase tracking-[0.2em] mb-4 text-accent-light"
                >
                  {section.heading}
                </h3>
                <ul className="space-y-2.5">
                  {section.links.map((link) => (
                    <li key={link.href}>
                      <Link
                        href={link.href}
                        className="text-sm transition-colors hover:text-accent text-slate-400"
                      >
                        {link.label}
                      </Link>
                    </li>
...
function SocialLink({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  return (
    <a
      ...
      aria-label={label}
      className="inline-flex h-9 w-9 items-center justify-center rounded-full transition-all duration-300 border border-brand-200/20 text-slate-400 hover:border-accent/50 hover:text-accent"
    >
      {children}
    </a>
  );
    ...
    <div>
      ...
      <form onSubmit={onSubmit} className="flex items-end gap-2">
        ...
        <button
          ...
          className="inline-flex h-9 w-9 items-center justify-center rounded-full text-brand-900 disabled:opacity-60 transition-all duration-300 bg-gradient-to-br from-accent to-accent-dark shadow-lg shadow-accent/30"
        >
