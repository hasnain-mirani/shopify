...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Remove Notification FCM permission prompt
      toast.success(`Hi ${name}, your account has been created. Happy shopping!`, { duration: 4000 });
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      toast.success(`Hi ${user.displayName || "there"}, great to have you onboard!`, { duration: 4000 });
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-brand-900/60 backdrop-blur-xl rounded-3xl border border-brand-200/10 p-8 shadow-[0_30px_80px_rgba(0,0,0,0.4)]">
          ...
          <p className="font-ui text-sm text-slate-400 mb-8">
            ...
            <Link href="/account/login" className="text-amber-400 hover:text-amber-300 font-semibold transition-colors">
              Sign in
            </Link>
          </p>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              <label className="block font-ui text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Full Name</label>
              <input
                type="text"
                required
                value={name}
            ...
            <div>
              <label className="block font-ui text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Password</label>
